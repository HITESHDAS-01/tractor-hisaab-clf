      ...
      <div className="flex flex-col h-full">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${
                  isActive
                    ? "bg-white/15 text-[var(--harvest)] font-semibold"
                    : "text-white/80 hover:bg-white/10 hover:text-white"
                }`}
              >
                <span className={isActive ? "text-[var(--harvest)]" : "text-white/80"}>
                  {item.icon}
                </span>
                <span className="text-sm">{t(item.key, lang)}</span>
              </Link>
            );
          })}
        </nav>
        ...
        <div className="px-3 py-4 border-t border-white/10">
          ...
          >
            ...
            <span className="text-sm">{t("logout", lang)}</span>
          </button>
        </div>
      </div>
    </aside>
